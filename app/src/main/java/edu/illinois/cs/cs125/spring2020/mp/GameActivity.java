...
public final class GameActivity extends AppCompatActivity {
    ...
    private void onLocationUpdate(final Location location) {
        ...
        if (game.getMyTeam() == OBSERVER) {
            return;
        }

        /* Package location in LatLng object */
        ...
        locUpdate.addProperty("latitude", coordinate.latitude);
        locUpdate.addProperty("longitude", coordinate.longitude);

        /* Once the object is ready, convert it to a JSON string and send it over the websocket */
        webSocket.sendText(locUpdate.toString());

        // Call the logic that updates game-play based on the user's movements
        if (gameState == GameStateID.RUNNING) {
            game.locationUpdated(coordinate);
        }
    }
        ...
        switch (type) {
            ...
            case "gameState":
                ...
                // Near the end of the checkpoint, add an else branch to handle the end of the game
                    ...
                    // Set message based on which team won
                    int winTeam = game.getWinningTeam();
                    switch (winTeam) {
                        case TEAM_RED:
                        ...
                        case TEAM_BLUE:
                            ...
                            break;
                        default:
                            throw new IllegalStateException("Unexpected value: " + winTeam);
                    }

                    // Dismissing the alert finished the activity
