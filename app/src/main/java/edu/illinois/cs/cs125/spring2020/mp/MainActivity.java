...
public final class MainActivity extends AppCompatActivity {
    ...
    private void setUpUi(final JsonObject result) {
        ...
        for (JsonElement g : games) {
            ...
            View chunk;
            if (myEmail != null && summary.isOngoing(myEmail)) {
                ...
                // Get buttons specific to ongoing games (enter)
                Button enter = chunk.findViewById(R.id.enterGame);
                // if a player clicks the enter button
                enter.setOnClickListener(w -> {
                    Intent intent = new Intent(this, GameActivity.class);
                    intent.putExtra("game", gameId);
                    startActivity(intent);
                });

                // Get buttons specific to ongoing games (leave)
                ...
                // If a player clicks on the leave button
                leave.setOnClickListener(v -> WebApi.startRequest(
                        this, WebApi.API_BASE + "/games/" + summary.getId() + "/leave",
                    Request.Method.POST, null, leaveState -> {
                ...
                // Get buttons specific to invitations (accept)
                Button accept = chunk.findViewById(R.id.acceptInvite);
                // If you click the accept button
                accept.setOnClickListener(v -> WebApi.startRequest(
                        this, WebApi.API_BASE + "/games/" + summary.getId() + "/accept",
                    Request.Method.POST, null, acceptedState -> {
                        if (summary.getId() != null) {
                            this.onRestart();
                        }
                    }, error -> Toast.makeText(this, error.getMessage(), Toast.LENGTH_LONG).show())
                ...
                // Get buttons specific to invitations (decline)
                Button decline = chunk.findViewById(R.id.declineInvite);
                // If you click the decline button

                decline.setOnClickListener(v -> WebApi.startRequest(
                        this, WebApi.API_BASE + "/games/" + summary.getId() + "/decline",
                    Request.Method.POST, null, declinedState -> {
                        if (summary.getId() != null) {
                            this.onRestart();
                        }
                    }, error -> Toast.makeText(this, error.getMessage(), Toast.LENGTH_LONG).show())
